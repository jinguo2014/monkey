...
def installApk(config):
    ...
    if phoneAddr:
        installPhoneApk = "adb -s %s install -r %s\\apk\\huatuo.apk" % (phoneAddr, WORKSPACE)
        os.popen(installPhoneApk)
        print ("install phone apk done")


def killTestApp():
    forceStopApp = "adb -s %s shell am force-stop com.huatuo_b2b.htb2b" % workConfig.get('phone')
    os.popen(forceStopApp)

def fullmonkey(workconfig):
    killTestApp()

    monkeycmd = "adb -s %s shell monkey -p com.huatuo_b2b.htb2b " \
